...
document.addEventListener('DOMContentLoaded', function() {
  ...
  const userNMessage = document.querySelector('.userN-message');
  ...
  userNInput.addEventListener('blur', function() {
    const userN = this.value;
    fetch('/check?userN=' + userN)
      .then(response => response.json())
      .then(result => {
        if (!result.available) {
          userNMessage.textContent = 'UserN is not available';
          userNMessage.style.color = 'red';
        } else {
          userNMessage.textContent = '';
          userNMessage.style.color = '';
        }
      })
      .catch(error => {
