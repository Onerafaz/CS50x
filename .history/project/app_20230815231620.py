        ...
        # Add rows into Database
            ...
            try:
                ...
                new_user = sql_command(
                    "INSERT INTO users (name, email, password) VALUES (?, ?, ?)", (ususer_nameer, user_email, password)
                )
            except:
...
def check_username():
    ...
    if user_email:
        rows = sql_query("SELECT email FROM users WHERE email = ? OR name = ?", (username, user_email))
